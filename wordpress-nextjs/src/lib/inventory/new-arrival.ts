 ...
 * `null` = off: any readable date qualifies. Set to e.g. 7 to badge only the
 * last week's stock. Affects the badge only — the "New Arrivals" sort always
 * ranks every vehicle by `LastUpdated`, latest first.
 */
export const NEW_ARRIVAL_WINDOW_DAYS: number | null = null;

const DAY_MS = 24 * 60 * 60 * 1000;

/**
 * A vehicle counts as a "New Arrival" when the feed gives it a readable
 * `LastUpdated` date (and, once {@link NEW_ARRIVAL_WINDOW_DAYS} is set, one
 * within that many days). Kept in one place so the VDP badge and the SRP card
